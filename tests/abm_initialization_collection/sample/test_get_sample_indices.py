...
class TestMakeGridCoordinates(unittest.TestCase):
    def test_get_sample_indices_rect_grid(self) -> None:
        bounds = (2, 6, 12)
        resolution = 2
        scale_xy = 1
        scale_z = 0.5
    ...
    def test_get_sample_indices_hex_grid(self) -> None:
        bounds = (2, 6, 12)
        resolution = 2
        scale_xy = 1
        scale_z = 0.5
    ...
    def test_get_sample_indices_invalid_grid_throws_exception(self) -> None:
        with self.assertRaises(ValueError):
            bounds = (0, 0, 0)
            grid = "invalid_grid"
            get_sample_indices(grid, bounds, 0, 0, 0)

    def test_get_rect_sample_indices(self) -> None:
        bounds = (2, 6, 12)
        resolution = 2
        scale_xy = 1
        scale_z = 0.5
    ...
    def test_get_hex_sample_indices(self) -> None:
        bounds = (2, 6, 12)
        resolution = 2
        scale_xy = 1
        scale_z = 0.5
